 ...
 * принимал его ОБРАТНО аргументом `drive_confirm_upload` — то есть ходил по
 * адресу, пришедшему от модели. Это классическая подделка запроса со стороны
 * сервера (SSRF): достаточно уговорить модель подставить туда
 * `http://169.254.169.254/…`, и сервер сам сходит во внутреннюю сеть и вернёт
 * ответ наружу.
 *
 ...
 * память процесса. Записи одноразово-долгоживущие: сессия Drive живёт около
 * недели, дольше хранить нечего.
 */
