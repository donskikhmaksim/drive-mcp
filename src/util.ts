...
/**
 * Экранирует значение, подставляемое внутрь строкового литерала Google Drive
 * query (`name='<значение>'`, `fullText contains '<значение>'` и т.п.).
 *
 * Зачем: Drive query — это язык с оператором `or`, и `and` в нём приоритетнее
 * `or`. Значение с одинарной кавычкой закрывает литерал и дописывает СВОЮ
 * дизъюнкцию, из которой можно выкинуть ограничение «только внутри такой-то
 * папки» — то есть выйти за пределы разрешённого поддерева (см.
 * `src/tools/skill_version.ts` и `references/security-checklist.md` §2:
 * идентификаторы/значения из аргументов инструмента ДОЛЖНЫ обезвреживаться
 * перед подстановкой во внешний запрос).
 *
 * Порядок замен важен: сначала обратный слэш, потом кавычка. Иначе значение,
 * кончающееся на `\`, экранирует уже НАШУ закрывающую кавычку и снова рвёт
 * литерал — типовая ошибка «экранируем только кавычку» (так делают инлайн
 * `src/tools/drive.ts:1405` и `src/tools/docs.ts:567`, их стоит перевести на
 * этот хелпер отдельным коммитом).
 *
 * Экранирование — НЕ единственный слой защиты и не должно им быть: вызывающий
 * код обязан дополнительно валидировать формат значения и ПОСЛЕ поиска
 * проверять, что найденный объект действительно лежит там, где разрешено.
 */
