 ...
 * (ТЗ `TZ_automation_key_consent_gate.md`, `TZ_automation_key_hub.md` —
 * единая генерация ключа живёт в gmail-mcp, эта ветка только ПРОВЕРЯЕТ его).
 * Валидный ключ (`automationKey` + DI `checkAutomationKey`, обе inject-able —
 * см. `RequireConsentParams` ниже) строит план и исполняет его немедленно,
 * минуя `user_reply`/ручное подтверждение — но НЕ минуя binding (`rehash`) и
 * кап батча (`cfg.sendBatchMax`): те же проверки, что и на человеческом пути.
 * `checkAutomationKey` не подключён (undefined) → эта ветка целиком не
 * существует, поведение побайтово как раньше — единственный вход доверия
 * снова только `user_reply`.
 */
...
export interface ConsentStore {
   ...
   * исполнитель тула), отчёт об исполнении (id файла, ссылка, пруф
   * post-verify) уходит в браузер человека, а модели доставалось только
   * «уже исполнено через другой канал» — то есть модель не знала, ЧТО именно
   * произошло. Реальный исход мутации лежит ровно в одном месте, доступном
   * серверу, — в `consent_audit.post_verify_result`, который заполняет сам
   * инструмент через `updateConsentAuditOutcome` выше.
   *
...
export interface RequireConsentParams<T = unknown> {
   ...
   * undefined/"" — обычный человеческий путь, ничего не меняется.
   */
   ...
   * поведение как раньше. Реализация живёт в `automation_key.ts`/store
 ...
 * безусловное «refused -> просто верни текст»), но модель получала машинный
 * сигнал «отказ» при тексте «✅ Подтверждено и исполнено» — и повторяла вызов
 * по кругу (жалоба Максима: «подтверждаю за 2 секунды, а он шлёт запрос
 * заново»).
 *
 ...
 * может «случайно» повторить мутацию (второй файл, второе расшаривание),
 * потому что данных для неё в исходе нет. `report` — готовый текст (превью +
 * факт исполнения), который тул обязан ретранслировать как ОТЧЁТ ОБ
 * ИСПОЛНЕНИИ (`okExecutionReport`), а не как отказ.
 */
...
/**
 * Докстринг параметра `automation_key` — навешивается на zod-параметр каждого
 * гейтованного инструмента (ТЗ `TZ_automation_key_consent_gate.md`, тот же дух,
 * что уже стоит в ticktick-mcp у аналогичного параметра). Английский текст —
 * намеренно, по спецификации: не угадывать формулировку заново.
 */
...
/** Метка вместо `user_reply` человека на automation_key-пути — честно отражает
 * происхождение (валидный ключ, не текстовая реплика), видна в аудит-логе.
 * Тот же приём, что `TG_AUTO_REPLY_MARKER` ниже, для той же цели: если этот
 * текст случайно попадёт в `classifyReply` где-то ещё, он не должен пройти
 * как настоящее «да». */
export const AUTOMATION_KEY_REPLY_MARKER = "[авто: automation_key]";
...
function inlineReply(s: string, max = 80): string {
  ...
  // клиент не может закодировать в UTF-8 (та же поломка, что уже чинилась в
  // `safeText` соседнего gmail-mcp). Стало важнее с 2026-08-14: через эту же
  // функцию теперь проходит пруф post-verify, где эмодзи статус-легенды есть
  // практически всегда.
  let out = "";
  for (const ch of one) {
 ...
 * несёт ПОЛНЫЙ URL, включая query, а presigned-ссылка Drive кладёт токен
 ...
 * становится DONE) и только ПОТОМ зовёт исполнителя тула, который делает саму
 * мутацию и дописывает пруф в аудит. Значит sync-wait почти всегда видит DONE
 * РАНЬШЕ, чем появился отчёт, — без короткого добора мы бы систематически
 * отдавали модели «результат неизвестен» ровно там, где он через секунду есть.
 */
...
function renderExecutionFact(
  ...
  if (audit.postVerifyResult) {
    ...
    // включает внешние куски (имя файла, адрес получателя доступа) — они не
    // должны разорвать блок отчёта переводом строки.
...
export async function requireConsent<T = unknown>(
  ...
  // ───── Automation-key быстрый путь (ТЗ automation_key_consent_gate.md) ─────
  // Проверяется ДО обычной hasId/hasReply развилки. Невалидный/просроченный/
  // не-по-scope ключ — НЕ ошибка, тихий fallthrough на обычный путь ниже
    ...
    if (akCheck.ok) {
      ...
      if (built.batchSize != null && built.batchSize > cfg.sendBatchMax) {
        return refuse(
          ...
          { batchCap: "exceeded", automationKey: akCheck.channel ?? "unknown" },
        );
      }
      // Тот же binding-чек, что и в обычной фазе исполнения ниже (шаг 4) — даже
      ...
      // `built.payload` играет роль АДРЕСАЦИИ здесь ровно так же, как
      // `row.payload` играет её в обычном пути — см. контракт `rehash` выше.
      ...
      if (currentHash !== built.objectHash) {
        return refuse(
          ...
          { binding: "mismatch", automationKey: akCheck.channel ?? "unknown" },
        );
      }
      const auditId = randomUUID();
      await store.appendConsentAudit({
        ...
        manifestId: null,
        objectHash: built.objectHash,
        userReply: AUTOMATION_KEY_REPLY_MARKER,
        checks: { automationKey: akCheck.channel ?? "unknown", binding: "ok", batchCap: "ok" },
        outcome: "confirmed",
        actor: "automation",
      });
      return { kind: "confirmed", manifestId: "", payload: built.payload as T, auditId };
    }
    // {ok: false} — падаем сквозь, ниже идёт обычный человеческий путь.
  }
  ...
  if (!hasId && !hasReply) {
    ...
    // исполнит её ЕЩЁ РАЗ — второе письмо, второй файл.
    //
    // Поэтому положительный исход отдаётся ОТДЕЛЬНЫМ исходом
    // `{kind: "already_executed", report: <готовый текст>}` — БЕЗ `payload`:
    // исполнять физически нечего, повторная мутация невозможна по типу, а не
    // по договорённости. До 2026-08-14 он ехал в форме `{kind:"refused"}` с
    // позитивным текстом внутри — защита работала, но модель получала
    // машинный сигнал «отказ» на успешное действие и звала инструмент по
    // кругу (жалоба Максима). Осознанное отступление от буквального
    // "kind: confirmed" из текста ТЗ сохраняется — оно и было про
    // безопасность, а не про метку.
    ...
    if (syncWaitMs > 0) {
      ...
      if (lastRow?.status === "DONE") {
        ...
        //
        // 2026-08-14: положительный исход больше НЕ маскируется под
        // `{kind:"refused"}` (см. развёрнутый комментарий выше и ConsentDecision).
        // Защита от двойного исполнения осталась ровно та же и стала строже:
        // исход `already_executed` НЕ НЕСЁТ `payload` — тулу физически нечем
        // повторить мутацию, это гарантируется типом, а не соглашением о том,
        // что каждый call site «не забудет» вернуть текст как есть.
        // Аудит здесь не пишем (его уже вёл тот, кто реально исполнил) —
        // только ЧИТАЕМ из него фактический результат для модели.
        ...
        // «✅ Подтверждено и исполнено» — тот же баг здесь, drive-mcp
        // побайтово переносит этот модуль): статус манифеста DONE ставится
        ...
        if (executionFailed) {
          ...
          // `outcome:"failed"`, см. `runAutoExecutePoller`/`/pending-consents/decide`).
          // НИКОГДА не ✅ и НИКОГДА безусловное «повторять не нужно» —
          // манифест уже одноразово потреблён (повтор С НИМ ничего не даст),
          // но был ли реальный эффект — неизвестно, это ДОЛЖНО быть
