 ...
 *
 * ЧТО ОБЯЗАН ДАТЬ ПРИНИМАЮЩИЙ РЕПОЗИТОРИЙ при переносе (иначе не соберётся —
 * оба поля объявлены ОБЯЗАТЕЛЬНЫМИ намеренно, чтобы «забыл» падало на сборке,
 * а не отключало защиту молча):
 *   • `ConsentStore.markTgNotified` — колонка/поле «план ушёл кнопкой»
 *     (в TS-репах это `consent_manifests.tg_notified`, миграция в store.ts);
 *   • `TgApprovalGate.hasAutoExecutor` — «есть чем исполнить по нажатию»
 *     (инжектится server.ts поверх `createTgApprovalGate`).
 *
 ...
 * сигнала «пришёл новый ход пользователя». Поэтому текстовый гейт ПОВЕДЕНЧЕСКИЙ, а
 * не криптографический: он превращает тихое само-подтверждение в ЯВНУЮ ЛОЖЬ,
 * которая остаётся в аудит-логе.
 *
 * ГДЕ ЭТОТ ПРЕДЕЛ УЖЕ ЗАКРЫТ (обновлено 2026-08-06): для плана, который РЕАЛЬНО
 * ушёл кнопкой в Telegram и у инструмента есть авто-исполнитель, текстовый путь
 * закрыт СОВСЕМ — см. `isTgButtonOnly` ниже. Такой план исполняется только
 * фоновым поллером после нажатия кнопки; кнопка модели недоступна, поэтому
 * сочинить согласие за человека для него нельзя вообще, а не «нежелательно».
 * Для остальных случаев (Telegram-слой выключен / у тула нет авто-исполнителя)
 * предел остаётся как описано выше — это честный компромисс, а не защита.
 * Целевая миграция: как только claude.ai получит form-mode elicitation —
 * брать подтверждение оттуда вместо `user_reply`, без смены сигнатур.
 *
