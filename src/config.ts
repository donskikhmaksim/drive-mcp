...
export interface ConsentGateConfig {
   ...
   * plan §0.4). `$self`, never a tool argument. Env CONSENT_SERVER, default "drive". */
  server: string;
   ...
   * 2000 — the generic gate.md default (NOT gmail's mail-specific 10000). */
...
export function loadConsentGateConfig(): ConsentGateConfig {
  return {
    server: process.env.CONSENT_SERVER?.trim() || "drive",
