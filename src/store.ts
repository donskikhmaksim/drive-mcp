...
export async function ensureSchema(): Promise<void> {
  ...
  `);
  // Short-lived download links handed to a client so it can fetch a file's
  // bytes itself (see downloads.ts). Rows are disposable — dropping the table
  // only invalidates links that were already about to expire.
  await p.query(`
  ...
  // come back IN from a tool argument — drive_confirm_upload takes an opaque
...
export interface DownloadTarget {
  /** Account label the file belongs to — resolved again when the link is used. */
  account: string;
  fileId: string;
  /** Export format for Google-native files; unset means "download the bytes as-is". */
  exportMime?: string;
  name: string;
  mimeType: string;
  size?: number;
...
export async function saveDownloadToken(token: string, t: DownloadTarget): Promise<void> {
  ...
  await p.query(
    `INSERT INTO download_tokens (token, account, file_id, export_mime, name, mime_type, size, expires_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
    [token, t.account, t.fileId, t.exportMime ?? null, t.name, t.mimeType, t.size ?? null, t.expiresAt],
  );
}
...
export async function getDownloadToken(token: string): Promise<DownloadTarget | null> {
  ...
  return {
    account: row.account,
    fileId: row.file_id,
    exportMime: row.export_mime ?? undefined,
    name: row.name,
...
export async function saveUploadSession(id: string, s: UploadSessionRecord): Promise<void> {
  ...
  await p.query(
    `INSERT INTO upload_sessions (id, account, upload_url, mode, file_id, name, mime_type, size_bytes, expires_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
    [id, s.account, s.uploadUrl, s.mode, s.fileId, s.name, s.mimeType, s.sizeBytes, s.expiresAt],
  );
}
...
export async function getUploadSession(id: string): Promise<UploadSessionRecord | null> {
  ...
  return {
    ...
    mode: row.mode === "replace" ? "replace" : "create",
...
// ($self, e.g. "drive") — every query filters on it so the 5 servers sharing
 ...
 *
 * Строк на один манифест может быть больше одной, поэтому сортировка сначала
...
/** Read path for A4's `drive_consent_audit` — "разбор инцидента без ssh"
...
/** Total rows matching `filters` (ignoring limit/offset) — lets `drive_consent_audit`
