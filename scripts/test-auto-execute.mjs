 ...
 * на бэке, не ждать повторного вызова моделью». Порт с gmail-mcp
 * (scripts/test-auto-execute.mjs, коммит 6e236d6). Фейковый ConsentStore,
...
const cfg = { server: "drive", consentTtlMs: 3_600_000, minConsentGapMs: 5_000, sendBatchMax: 10, now };
...
const PAYLOAD = { account: "work", folders: [{ name: "Отчёты 2026" }] };
...
console.log("[1] happy path — манифест AWAITING_CONSENT, binding совпадает → исполняется");
{
  ...
  await store.createManifest({
    id: "m1", server: "drive", tool: "drive_create_folder", accountLabel: "work",
    payload: PAYLOAD, objectHash: OBJHASH, createdAt: clock.t, expiresAt: clock.t + 3_600_000,
  });
  const result = await tryAutoExecute({ manifestId: "m1", tool: "drive_create_folder", accountLabel: "work" }, rehashOk, store, cfg);
...
{
  const store = makeStore();
  const result = await tryAutoExecute({ manifestId: "nope", tool: "drive_create_folder", accountLabel: "work" }, rehashOk, store, cfg);
...
console.log("\n[3] tool/accountLabel не совпадают с манифестом → null (защита от подмены)");
{
  const store = makeStore();
  await store.createManifest({
    id: "m3", server: "drive", tool: "drive_create_folder", accountLabel: "work",
    payload: PAYLOAD, objectHash: OBJHASH, createdAt: clock.t, expiresAt: clock.t + 3_600_000,
  });
  const wrongTool = await tryAutoExecute({ manifestId: "m3", tool: "drive_rename", accountLabel: "work" }, rehashOk, store, cfg);
  check("другой tool → null", wrongTool === null);
  const wrongAccount = await tryAutoExecute({ manifestId: "m3", tool: "drive_create_folder", accountLabel: "personal" }, rehashOk, store, cfg);
...
console.log("\n[4] уже исполнен (гонка/повторный тик поллера) → null, повторно не исполняется");
{
  const store = makeStore();
  await store.createManifest({
    id: "m4", server: "drive", tool: "drive_create_folder", accountLabel: "work",
    payload: PAYLOAD, objectHash: OBJHASH, createdAt: clock.t, expiresAt: clock.t + 3_600_000,
  });
  const first = await tryAutoExecute({ manifestId: "m4", tool: "drive_create_folder", accountLabel: "work" }, rehashOk, store, cfg);
  check("первый вызов исполнился", first !== null);
  const second = await tryAutoExecute({ manifestId: "m4", tool: "drive_create_folder", accountLabel: "work" }, rehashOk, store, cfg);
  check("второй вызов (та же гонка) → null, папка не создастся дважды", second === null);
...
console.log("\n[5] дрейф состояния (binding mismatch) → null, манифест НЕ инвалидируется молча");
{
  const store = makeStore();
  await store.createManifest({
    id: "m5", server: "drive", tool: "drive_create_folder", accountLabel: "work",
    payload: PAYLOAD, objectHash: OBJHASH, createdAt: clock.t, expiresAt: clock.t + 3_600_000,
  });
  const result = await tryAutoExecute({ manifestId: "m5", tool: "drive_create_folder", accountLabel: "work" }, rehashDrift, store, cfg);
...
console.log("\n[6] TTL истёк → null (тот же guard, что у обычного consumeManifest)");
{
  const store = makeStore();
  await store.createManifest({
    id: "m6", server: "drive", tool: "drive_create_folder", accountLabel: "work",
    payload: PAYLOAD, objectHash: OBJHASH, createdAt: clock.t - 4_000_000, expiresAt: clock.t - 1_000,
  });
  const result = await tryAutoExecute({ manifestId: "m6", tool: "drive_create_folder", accountLabel: "work" }, rehashOk, store, cfg);
