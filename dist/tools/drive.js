...
function defaultExportMime(mime) {
...
export function registerDriveTools(server, clients) {
    const account = accountField(clients);
    server.registerTool("drive_search", {
        title: "Search Drive files",
        description: "Search files/folders in Drive. Either pass a raw Drive `query` (Drive API q syntax) or use `nameContains`/`mimeType` helpers.",
        inputSchema: {
            account,
            query: z
                .string()
        ...
        if (!q) {
            ...
            if (nameContains)
                parts.push(`name contains '${nameContains.replace(/'/g, "\\'")}'`);
            if (mimeType)
    ...
    server.registerTool("drive_get_metadata", {
        ...
        const res = await g.drive.files.get({
            fileId,
            fields: "id,name,mimeType,modifiedTime,createdTime,size,owners(emailAddress),webViewLink,parents,trashed",
        });
    ...
    server.registerTool("drive_trash", {
        ...
        description: "Move a file to the trash (reversible). This does NOT permanently delete it. " +
        ...
        const g = clients.resolve(account);
        let before = _fileName ? { name: _fileName } : null;
        try {
    ...
    server.registerTool("drive_download_file", {
        ...
        description: "Read a Drive file's content. Text files are returned as text; Google Docs/Sheets/Slides are exported " +
        ...
        if (mime.startsWith("application/vnd.google-apps.")) {
            ...
            const res = await g.drive.files.export({ fileId, mimeType: outMime }, { responseType: "arraybuffer" });
        ...
        if (buf.length > limit) {
            return fail(`File is ${buf.length} bytes (binary) — too large to inline. Raise maxBytes (max 8MB) ` +
    ...
    server.registerTool("drive_upload_file", {
        title: "Upload / create a new Drive file",
        description: "Create a new file in Drive from provided content. " +
    ...
    server.registerTool("drive_overwrite_file", {
        title: "Overwrite an existing Drive file",
        description: "Replace the content of an existing Drive file by its id. " +
    ...
    server.registerTool("drive_extract_text", {
        title: "Extract text from a PDF/image (OCR)",
        description: "Extract text from a Drive file (PDF, scan, or image) using Google Drive's built-in OCR. " +
